...
async function main() {
    ...
    try {
        ...
        const storeAssertionScript = [
            ...
            "  const loadedImages = Array.from(document.images).filter(isVisible).filter((img) => img.complete && img.naturalWidth > 0);",
            ...
            "  const containedTextOverflow = Array.from(document.querySelectorAll('.addon-item > *, .price-tier > *, .price-summary-copy, .price-summary-action, .process-card h3, .process-card p, .shop-price')).filter(isVisible).map((node) => {",
            "    const boundary = node.closest('.addons-panel, .price-card, .process-card, .gallery-proof-card, .custom-order-card, .mallorca-proof-card');",
            "    if (!boundary) return '';",
            "    const rect = node.getBoundingClientRect();",
            "    const box = boundary.getBoundingClientRect();",
            "    const escaped = rect.left < box.left - 3 || rect.right > box.right + 3;",
            "    return escaped ? `${node.tagName.toLowerCase()}.${node.className || 'text'} escaped ${boundary.className}: ${node.textContent.replace(/\\s+/g, ' ').trim().slice(0, 80)}` : '';",
            ...
            "  if (!styles.some((src) => src.includes('store-warm.css?v=20260625-overflow-fix'))) issues.push('warm store css cache-bust marker missing');",
            ...
            "  if (loadedImages.length < 6 && placeholders.length < 6) issues.push(`expected loaded store images or visible labeled placeholders, got ${loadedImages.length} images and ${placeholders.length} placeholders`);",
            "  for (const selector of removedSections) {",
            "    const node = document.querySelector(selector);",
            "    if (node) issues.push(`${selector} should be removed from the public store`);",
            "  }",
            "  if (containedTextOverflow.length) issues.push(`card-contained text overflow: ${containedTextOverflow.join(' | ')}`);",
            "  if (!scripts.some((src) => src.includes('site-data.js?v=20260625-premium-motion-3'))) issues.push('site-data cache-bust marker missing');",
            "  if (!scripts.some((src) => src.includes('script.js?v=20260625-premium-motion-3'))) issues.push('script cache-bust marker missing');",
            "  if (overflow > 1) issues.push(`horizontal overflow ${overflow}px`);",
            "  return { shopHeading: text('#shop-heading'), shopCards: titles.length, cardTitles: titles, placeholders, loadedImages: loadedImages.length, containedTextOverflow, trustCockpitVisible: isVisible(trustCockpit), trustCockpitHeading: text('#trust-cockpit-heading'), overflow, issues };",
            "})()"
        ].join("\n");
        const portfolioAssertionScript = [
            ...
            "  const styles = Array.from(document.styleSheets).map((sheet) => sheet.href || '');",
            "  const overflow = Math.round(Math.max(document.documentElement.scrollWidth, document.body.scrollWidth) - document.documentElement.clientWidth);",
            "  const containedTextOverflow = Array.from(document.querySelectorAll('.portfolio-work-card *, .portfolio-social-card *, .portfolio-cred-grid article *, .portfolio-hero-copy *, .portfolio-hero-card *')).filter(isVisible).map((node) => {",
            "    const boundary = node.closest('.portfolio-work-card, .portfolio-social-card, .portfolio-cred-grid article, .portfolio-hero-copy, .portfolio-hero-card');",
            "    if (!boundary) return '';",
            "    const rect = node.getBoundingClientRect();",
            "    const box = boundary.getBoundingClientRect();",
            "    const escaped = rect.left < box.left - 3 || rect.right > box.right + 3;",
            ...
            "  if (!scripts.some((src) => src.includes('site-data.js?v=20260625-premium-motion'))) issues.push('portfolio site-data cache-bust marker missing');",
            "  if (!scripts.some((src) => src.includes('portfolio.js?v=20260625-premium-motion-2'))) issues.push('portfolio script cache-bust marker missing');",
            ...
            "  if (!styles.some((src) => src.includes('portfolio-motion.css?v=20260625-premium-motion-2'))) issues.push('portfolio motion css marker missing');",
            ...
            "  if (containedTextOverflow.length) issues.push(`portfolio card-contained text overflow: ${containedTextOverflow.join(' | ')}`);",
            "  if (overflow > 1) issues.push(`horizontal overflow ${overflow}px`);",
            "  return { portfolioHeading: text('#portfolio-heading'), workCards, socialCards, previewSocialCards, credCards, containedTextOverflow, overflow, issues };",
            "})()"
        ].join("\n");
        const specs = [
            {
            ...
            {
                key: "storeTablet",
                label: "Store tablet overflow check",
                url: `${baseUrl}/?preview=store&v=${cacheBust}`,
                file: "maryilu-store-tablet-768.png",
                width: 768,
                initialHeight: 900,
                fullPage: false,
                assertionScript: storeAssertionScript
            ...
            {
                key: "storeSmallDesktop",
                label: "Store small desktop overflow check",
                url: `${baseUrl}/?preview=store&v=${cacheBust}`,
                file: "maryilu-store-small-desktop-1024.png",
                width: 1024,
                initialHeight: 900,
                fullPage: false,
                assertionScript: storeAssertionScript

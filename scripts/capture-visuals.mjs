...
async function main() {
    ...
    try {
        ...
        const storeAssertionScript = [
            ...
            "  if (!styles.some((src) => src.includes('store-warm.css?v=20260625-premium-motion'))) issues.push('warm store css cache-bust marker missing');",
            ...
            "  return { shopHeading: text('#shop-heading'), shopCards: titles.length, cardTitles: titles, placeholders, loadedImages: loadedImages.length, trustCockpitVisible: isVisible(trustCockpit), trustCockpitHeading: text('#trust-cockpit-heading'), overflow, issues };",
            "})()"
        ].join("\n");
